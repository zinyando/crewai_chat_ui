    ...
    try:
        ...
        for name, obj in inspect.getmembers(module):
            ...
            if (inspect.isclass(obj) and 
                name != "Flow" and  # Skip the base Flow class
                hasattr(obj, "__module__") and 
                obj.__module__ == module_name):
                
                # Try to determine if this is a Flow class by checking for typical attributes/methods
                if (hasattr(obj, "run") and inspect.isfunction(getattr(obj, "run")) and
                    (hasattr(obj, "nodes") or hasattr(obj, "add_node"))):
        ...
        logger.error(f"Error extracting flows from {file_path}: {str(e)}")
