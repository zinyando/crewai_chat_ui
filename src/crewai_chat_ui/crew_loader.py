...
def find_crew_module():
    ...
    print(f"Searching for crew files in: {current_dir}")
    ...
    if not potential_crew_files:
        print("No crew files found in user project directories. Looking for any .py files that might contain Crew instances...")
        ...
        if filtered_files:
            print(f"Found {len(filtered_files)} Python files to check in user project directories")
            potential_crew_files = filtered_files[:5]  # Limit to first 5 to avoid excessive search
    
    # If we still have no user project files but have system files, use the system files as a last resort
    if not potential_crew_files:
        print("No user project files found. Falling back to any available crew files, including system libraries...")
    ...
    print(f"Potential crew files found: {[str(f) for f in potential_crew_files]}")
...
def load_crew() -> Tuple[Crew, Optional[str]]:
    ...
    try:
        ...
        print(f"Found crew file: {crew_path}")
        ...
        print("Looking for direct Crew instances...")
        ...
        for var_name in common_crew_vars:
            if hasattr(module, var_name):
                ...
                print(f"  Checking common variable: {var_name}, type: {type(attr).__name__}")
                if isinstance(attr, Crew):
                    crew_instance = attr
                    crew_name = var_name
                    print(f"  Found Crew instance in common variable: {var_name}")
                    break
        ...
        if crew_instance is None:
            for attr_name in dir(module):
                ...
                print(f"  Checking attribute: {attr_name}, type: {type(attr).__name__}")
                if isinstance(attr, Crew):
                    crew_instance = attr
                    crew_name = attr_name
                    print(f"  Found Crew instance: {attr_name}")
                    break
        
        if crew_instance is None:
            print("No direct Crew instance found. Looking for functions that return Crew...")
            # First check common function names
            common_func_names = ["get_crew", "create_crew", "build_crew", "init_crew", "setup_crew", "make_crew"]
            ...
            if hasattr(module, "crew") and callable(getattr(module, "crew")):
                ...
                print(f"  Found function named 'crew', trying to call it")
                try:
                    result = crew_func()
                    print(f"    Function 'crew' returned: {type(result).__name__}")
                    if isinstance(result, Crew):
                        crew_instance = result
                        crew_name = "crew"
                        print(f"    Found Crew from function named 'crew'")
                except Exception as func_error:
                    print(f"    Error calling 'crew' function: {str(func_error)}")
            ...
            if crew_instance is None:
                for func_name in common_func_names:
                    if hasattr(module, func_name):
                        ...
                        if callable(attr):
                            print(f"  Trying common function: {func_name}")
                            try:
                                result = attr()
                                ...
                                if isinstance(result, Crew):
                                    crew_instance = result
                                    crew_name = func_name
                                    print(f"    Found Crew from common function: {func_name}")
                                    break
                            except Exception as func_error:
                                print(f"    Error calling {func_name}: {str(func_error)}")
            ...
            if crew_instance is None:
                for attr_name in dir(module):
                    if attr_name.startswith("__"):  # Skip built-in functions
                        continue
                    attr = getattr(module, attr_name)
                    if callable(attr) and attr_name.lower().startswith(("get_", "create_", "build_", "init_", "setup_", "make_")):
                        print(f"  Trying function: {attr_name}")
                        try:
                            result = attr()
                            print(f"    Function returned: {type(result).__name__}")
                            if isinstance(result, Crew):
                                crew_instance = result
                                crew_name = attr_name
                                print(f"    Found Crew from function: {attr_name}")
                                break
                        except Exception as func_error:
                            print(f"    Error calling {attr_name}: {str(func_error)}")
                            continue
            ...
            for attr_name in dir(module):
                ...
                if isinstance(attr, type):  # Check if it's a class
                    ...
                    try:
                        ...
                        instance = attr()
                        print(f"  Instantiated class: {attr_name}")
                        
                        # Look for crew methods on the instance
                        if hasattr(instance, 'crew') and callable(getattr(instance, 'crew')):
                            print(f"  Found crew method on class {attr_name}")
                            try:
                                result = instance.crew()
                                print(f"  Method returned: {type(result).__name__}")
                                if isinstance(result, Crew):
                                    crew_instance = result
                                    crew_name = f"{attr_name}.crew"
                                    print(f"  Found Crew from method: {attr_name}.crew")
                                    break
                            except Exception as method_error:
                                print(f"  Error calling {attr_name}.crew: {str(method_error)}")
                    except Exception as instance_error:
                        print(f"  Error instantiating class {attr_name}: {str(instance_error)}")
                        continue

        if crew_instance is None:
            print("Could not find any Crew instance or function returning Crew")
            # Debug output: Show all module attributes
